...
@injectable()
export class Barrier1 extends Component {
    ...
    public create(): void {
        this.events.on('barrier1', (event: Phaser.Types.Tilemaps.TiledObject) => {
            this.barrier = this.scene.add.sprite((event.x + event.width * this.constants.HALF) * this.constants.SCALE, (event.y - event.height * this.constants.HALF) * this.constants.SCALE, 'textures', 'barrier1');
            this.scene.physics.add.existing(this.barrier, true);
            (this.barrier.body as Phaser.Physics.Arcade.Body).setSize(this.barrier.width, this.barrier.height * 2);
            this.barrier.setScale(this.constants.SCALE);
            this.barrier.setDepth(this.constants.MAP_DEPTH);
        ...
        this.events.on('lightBrazier1', (brazier: Phaser.GameObjects.Sprite) => {
            ...
            setTimeout(() => {
                this.events.fire('playAudio', { key: 'music1', volume: 0.7 });
            }, 500);
