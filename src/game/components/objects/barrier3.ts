...
@injectable()
export class Barrier3 extends Component {
    ...
    public create(): void {
        this.events.on('barrier3', (event: Phaser.Types.Tilemaps.TiledObject) => {
            this.barrier = this.scene.add.sprite((event.x + event.width * this.constants.HALF) * this.constants.SCALE, (event.y - event.height * this.constants.HALF) * this.constants.SCALE, 'textures', 'barrier3');
